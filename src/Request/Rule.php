...
use Hyperf\Validation\Rule as HyperfRule;
use Rainte\Hyperf\Model\Model;
...
class Rule
{
     ...
     * @return string
     */
    public static function phone(bool $required = false): string
    {
        $str = static::required($required);

        return $str . 'string|regex:/^1[3456789]{1}\d{9}$/';
    }
    ...
    public static function page(): array
    {
        return [
            ...
            'status' =>  HyperfRule::in(Model::STATUS_ALL),
            config('rainte.SORT_AT') => 'string|ends_with:asc,desc',
            config('rainte.PAGE_NO_AT') => 'integer|numeric|min:1',
            config('rainte.PAGE_SIZE_AT') => 'integer|numeric|min:1|max:100',
        ];
    }
